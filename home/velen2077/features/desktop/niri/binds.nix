{
  ...
  programs.niri.settings = {
    ...
    in {
      ...
      #      "Mod+O".action.spawn = ["cliphist-paste"];
      "Mod+Shift+C".action = sh "env DISPLAY=:1 xsel -ob | wl-copy";
      "Mod+Shift+V".action = sh "wl-paste -n | env DISPLAY=:1 xsel -ib";
      ...
      # Setup first workspace as a sort of scratchpad workspace
      # similar to paperwm
      "Mod+1".action.spawn = [
        "bash"
        "-c"
        ''
          ACTIVE_WORKSPACE=$(niri msg --json workspaces | jq '.[] | select(.is_active == true) | .idx')
